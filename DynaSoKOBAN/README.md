...
後端原始碼已整合到 `../G2B3/gas/Dynasty.js`，由共用入口依 `unitName: DynaSoKOBAN` 分流。舊版前端傳送 Google ID token 的 API 仍保留；目前正式入口改為免登入遊玩，通關後使用 DynaSoKOBAN 專屬 Google Form 登錄成績並上傳通關證書截圖。
...
直接開啟 `file://` 可進行遊戲；成績登錄時仍須開啟 Google Form 並登入 Google 帳號。
