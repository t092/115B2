...
class Battle{
 constructor(state){this.state={...state};this.startState={...state};this.route=routeFor(state);this.time=0;this.timeLimit=state.chapter===0?40:state.chapter<3?60:Infinity;this.phase='ready';this.units=[];this.hazards=this.route.hazards.map(([x,type],id)=>({x,type,id,nextShot:0,shot:null}));this.pending=0;this.deliveries=[];this.spent=0;this.losses=0;this.nextId=1;this.cooldown={};this.events=[];this.settled=false;}
 begin(){if(this.phase==='ready')this.phase='running';}
 ...
 step(dt){if(this.phase!=='running'||!Number.isFinite(dt)||dt<=0)return;let left=this.state.chapter<3?Math.min(dt,Math.max(0,this.timeLimit-this.time)):dt;while(left>1e-9){const n=Math.min(.025,left);this.tick(n);left-=n;}if(this.state.chapter<3&&this.time>=this.timeLimit-1e-8){this.time=this.timeLimit;this.phase='finished';}}
 ...
 settle(){if(this.settled)return null;if(this.phase!=='finished'&&!this.canSettle())return null;this.settled=true;this.phase='result';this.state.money+=this.pending;const report={chapter:this.state.chapter,origin:this.route.origin,destination:this.route.destination,starting:this.startState.money,spent:this.spent,passed:this.deliveries.length,goods:this.deliveries.reduce((s,a)=>s+a.goods,0),revenue:this.pending,balance:this.state.money,losses:this.losses,undelivered:this.units.filter(a=>a.alive&&!a.arrived&&TYPES[a.key].goods).length};return report;}
 advance(){if(!this.settled)return null;const s={...this.state};if(s.chapter<3){s.chapter++;if(s.chapter===3)s.baseline=s.money;}else{if(s.leg==='back'&&s.money-s.baseline>=10000)s.won=true;s.leg=s.leg==='out'?'back':'out';if(s.leg==='out')s.round++;}return s;}
}
