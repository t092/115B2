...
function enter(p){profile=p;box.close();$('playerLabel').textContent=p?`${p.classId} 班 ${p.seatNo} 號 · ${p.name}`:'訪客遊玩';record=p?read(key()):null;status(p?'最終總金額就是分數；第四關結束後送交教師核對。':'訪客模式：可以遊玩，不會紀錄成績。');$('retryScore').hidden=!(record&&!record.sent);resolveReady();}
...
}catch(err){const message=/permission|insufficient/i.test(err.message)?'Firebase 拒絕寫入（權限不足）。請確認雲端規則已允許 silklemmings 與金額分數。':err.message;status(`成績尚未送出：${message} 最終分數已暫存，可按「重試送出」。`);}finally{busy=false;$('retryScore').disabled=false;$('retryScoreDialog').disabled=false;$('retryScore').hidden=!!record?.sent;$('retryScoreDialog').hidden=!!record?.sent;}}
$('retryScore').onclick=upload;$('retryScoreDialog').onclick=upload;
function complete(money,runId){$('scoreReceipt').hidden=false;if(!profile){status(`訪客總結算：${money.toLocaleString()} 分，不紀錄成績。`);return;}if(record?.id===runId){if(record.sent)status(`已送出 ${record.score.toLocaleString()} 分，等待教師核對。`);else upload();return;}record={id:runId,score:Math.max(0,Math.floor(money)),sent:false};write(key(),record);upload();}
