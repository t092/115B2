...
(async()=>{
 ...
 try{
  await page.route('https://www.gstatic.com/**',r=>r.abort());await page.goto(pathToFileURL(path.resolve(__dirname,'../../index.html')).href+'?test');await page.locator('#guestPlay').click();await page.waitForFunction(()=>silkTest.get().imagesReady===6);await start();
  ...
  await start({chapter:3,money:1000,baseline:1000,leg:'out'});await direction(true);await page.evaluate(()=>silkTest.step(61));assert.equal(await page.locator('#dialog').evaluate(e=>e.open),false);assert.equal(await page.locator('#timer').textContent(),'∞');
  ...
  await page.reload();await page.locator('#guestPlay').click();await page.waitForFunction(()=>silkTest.get().imagesReady===6);assert.match(await page.locator('#originLabel').textContent(),/大秦/);
  assert.deepEqual(errors,[]);const result={passed:true,checks:['six sprite sheets including archer loaded from local files','no preparation sidebar','origin-specific visible cards','single click purchase','Chang’an departures move right, returns move left','battlefield click does not purchase','pause freezes clock','40-second first chapter settlement and carried balance','chapter four unlimited','mobile 390 and 320 widths','mobile single-tap purchase','saved city reload'],errors};fs.writeFileSync(path.join(__dirname,'browser-results.json'),JSON.stringify(result,null,2));console.log(JSON.stringify(result,null,2));
