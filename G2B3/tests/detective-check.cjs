...
const output=path.resolve(__dirname,'../../test-results');require('node:fs').mkdirSync(output,{recursive:true});
(async()=>{const browser=await chromium.launch({headless:true,channel:'msedge',args:['--use-angle=swiftshader','--enable-unsafe-swiftshader']});try{
 ...
 const page=await create(1);await page.locator('#stageDetective').screenshot({path:path.join(output,'detective-before.png')});
 ...
 await page.locator('#stageDetective').screenshot({path:path.join(output,'detective-results.png')});
 ...
 await mobile.locator('#stageDetective').screenshot({path:path.join(output,'detective-mobile.png')});
