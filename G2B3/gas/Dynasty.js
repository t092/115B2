// Shared GAS module; deployed only with G2B3/gas.
var Dynasty = (function() {
/**
 ...
 * POST 請求處理：提交學生成績
 */
function submit(payload, identity) {
  try {
    if (!payload.classId || !payload.name || !/^\d{1,3}$/.test(String(payload.seatNo || ''))) throw new Error('請填寫班級、座號與姓名');
    for (const key of ['totalScore', 'moves', 'durationSeconds', 'maxLevel']) {
      if (!Number.isSafeInteger(payload[key]) || payload[key] < 0 || payload[key] > 1000000) throw new Error('成績欄位格式錯誤');
    }
    if (payload.maxLevel < 1 || payload.maxLevel > 6 || !Array.isArray(payload.levelDetails) || JSON.stringify(payload.levelDetails).length > 20000) throw new Error('關卡資料格式錯誤');
    const ss = getSpreadsheet();
    initSheets(ss);
    ...
    if (action === 'submitScore') {
      ...
      const name = sheetText(payload.name);
      const classId = sheetText(payload.classId);
      const seatNo = sheetText(payload.seatNo);
...
return { submit: submit, get: doGet };
})();
